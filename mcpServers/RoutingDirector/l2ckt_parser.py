...
def parse_l2circuit_json(json_data):
    """
    Simple function to parse L2 circuit services from JSON data
    Returns: DataFrame with L2 circuit services and reference data dictionary
    """
    
    def extract_devices(l2vpn_ntw):
        """Extract device ne_id and vpn_node_id pairs"""
        devices = []
        try:
            ...
            for vpn_service in vpn_services.get('vpn_service', []):
                ...
                for node in vpn_nodes.get('vpn_node', []):
                    ...
                    vpn_node_id = node.get('vpn_node_id', '')
                    if ne_id and vpn_node_id:
                        devices.append(f"{ne_id}({vpn_node_id})")
        except:
            pass
        return devices
    ...
    for item in data_list:
        # Only process L2 circuit services
        if item.get('design_id') == 'eline-l2circuit-nsm':
            ...
            service_record = {
                'L2 Circuit Service Name': service_name,
...
if __name__ == "__main__":
    ...
    # Parse L2 circuit services
    services_df, ref_data = parse_l2circuit_json(data)
    print("*"*60)
    print(services_df)
    print("*"*60)
    
    # Display results
    print("=== L2 Circuit Services ===")
    if not services_df.empty:
        print(services_df.to_string(index=False))
        print(f"\nTotal Services: {len(services_df)}")

        services_df.to_csv("l2_circuit_services.csv", index=False)
        print("✅ Services saved to l2_circuit_services.csv")

        # Example: Access reference data for LLM
        for service_name in services_df['L2 Circuit Service Name']:
            ...
            if service_name in ref_data:
                ...
                fh_config = ref_data[service_name]['fh_config']
                placement = ref_data[service_name]['placement']
    else:
        print("No L2 circuit services found")